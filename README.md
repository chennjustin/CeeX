...
一個以 Twitter / X 為靈感打造的社交媒體平台，使用 Next.js 14、Prisma、PostgreSQL、NextAuth、Pusher、Cloudinary 等技術實作，提供註冊、登入、發文、追蹤、通知、即時更新，也有支援**手機版響應式設計（RWD）**

### Demo / 線上體驗

- **部署連結**：**拉拉拉**
  - 建議使用桌機 Chrome / Edge，或手機瀏覽器直接操作
  - 如遇登入問題，可先重新整理或清除 Cookie 後再試一次
...
- ✅ **手機版優化**
  ...
  - **漢堡選單**：
    ...
    - 側邊欄頂部顯示 "CeeX" 標誌，右側顯示 X 關閉按鈕（與標誌同一排）
    - 點擊導航項目後自動關閉選單
